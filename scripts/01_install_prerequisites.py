...
# Default project root is two levels above this script
...
def log(msg):
    print(f"{GREEN}[{_ts()}] OK  {msg}{NC}")


def warn(msg):
    print(f"{YELLOW}[{_ts()}] !!  {msg}{NC}")


def info(msg):
    print(f"{CYAN}[{_ts()}]    {msg}{NC}")
...
def prompt(var_name, label, example, default=""):
    ...
    if current:
        info(
            f"Using {var_name}={current} "
            f"(pre-set in environment, skipping prompt)"
        )
        return current
...
print("    1. EKS cluster name")
print("    2. AWS region")
print("    3. VPC ID")
print("    4. ALB controller role ARN")
print()
...
GITOPS_PATH = prompt(
    "GITOPS_PATH",
    "Local path to your gitops repo",
    default_gitops,
    default_gitops
)
...
if not VPC_ID:
    info(
        f"Auto-fetching VPC ID for cluster "
        f"'{CLUSTER_NAME}'..."
    )
    ...
    if rc == 0 and VPC_ID and VPC_ID != "None":
        ...
        VPC_ID = prompt(
            "VPC_ID",
            "VPC ID where the EKS cluster runs",
...
# ---------------------------------------------------------------------------
info(
    f"Updating kubeconfig for cluster "
...
# ---------------------------------------------------------------------------
info("Testing Kubernetes API connectivity...")
...
ALB_VALUES_FILE = os.path.join(
    GITOPS_PATH,
    "k8s/ingress/alb-controller-values.yaml"
)
...
alb_cmd = [
    ...
    "--set",
    f"serviceAccount.annotations.eks\\.amazonaws\\.com/role-arn={ALB_CONTROLLER_ROLE}",
...
log("AWS Load Balancer Controller Helm release is deployed.")


# ---------------------------------------------------------------------------
# Wait for ALB controller deployment
# ---------------------------------------------------------------------------
info("Waiting for AWS Load Balancer Controller pods...")
...
log("AWS Load Balancer Controller deployment is ready.")


# ---------------------------------------------------------------------------
# Verify ALB webhook service
# ---------------------------------------------------------------------------
info("Checking AWS Load Balancer Controller webhook service...")
...
if rc != 0:
    die(
        "AWS Load Balancer Controller webhook service "
        "was not found."
    )

log("ALB webhook service exists.")


# ---------------------------------------------------------------------------
# Verify webhook endpoints
# ---------------------------------------------------------------------------
info("Waiting for ALB webhook endpoints...")
...
if not webhook_ready:
    die(
        "AWS Load Balancer Controller webhook has "
        "no ready endpoints."
    )

log(f"ALB webhook endpoints: {output}")


# ---------------------------------------------------------------------------
# Verify webhook TLS Secret
# ---------------------------------------------------------------------------
info("Checking ALB webhook TLS secret...")
...
if rc != 0:
    die(
        "AWS Load Balancer Controller TLS secret "
        "aws-load-balancer-tls was not found."
    )

log("ALB webhook TLS secret exists.")


# ---------------------------------------------------------------------------
# Verify webhook CA bundle
# ---------------------------------------------------------------------------
info("Checking ALB webhook CA bundle...")

ca_bundle, rc = run_cmd(
    [
        "kubectl",
        "get",
...
if rc != 0 or not ca_bundle:
    die(
        "AWS Load Balancer Controller webhook "
        "CA bundle is missing."
    )

log("ALB webhook CA bundle is present.")


# ---------------------------------------------------------------------------
# Give webhook certificate watcher time to stabilize
# ---------------------------------------------------------------------------
info(
    "Allowing the ALB webhook certificate watcher "
    "to stabilize..."
)

time.sleep(10)

log("AWS Load Balancer Controller webhook is ready.")
...
#
# Helm/controller manage these resources.
# ---------------------------------------------------------------------------
...
log("ArgoCD Helm release deployed.")
...
# ---------------------------------------------------------------------------
argocd_password_b64, rc = run_cmd(
    [
        "kubectl",
        "-n",
        "argocd",
...
if rc != 0 or not argocd_password_b64:
    warn(
        "Could not retrieve ArgoCD initial admin password."
    )
    argocd_password = "<retrieve manually>"
else:
    argocd_password = (
        base64.b64decode(argocd_password_b64)
        .decode()
        .strip()
    )
...
# ---------------------------------------------------------------------------
info("Waiting for External Secrets Operator...")
...
print()
print("AWS Load Balancer Controller webhook:")
run_cmd(
    [
        "kubectl",
        "get",
...
print()
log("All Kubernetes prerequisites installed successfully.")

print()
print("  Summary:")
print("    ALB controller   : READY")
print("    ALB webhook      : READY")
print("    ArgoCD           : READY")
print("    External Secrets : READY")
print(f"    ArgoCD pass      : {argocd_password}")
print()
print("  ALB hostnames will appear in:")
print("    kubectl get ingress -n <env>")
print()
